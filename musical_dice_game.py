...
def roll_dice(num = 1):#means that defaul num value is 1, unless specified as something else
    added = 0
    for x in range(num):
...
def construct_waltz():
    waltzmsrs = []
    for x in range(16):
        waltzmsrs.append(minuet_filename(minuet_table[x][roll_dice(2)]))
    for x in range(16):
	    waltzmsrs.append(trio_filename(trio_table[x][roll_dice(1)]))
    waltzfiles = []
    for name in waltzmsrs:
        waltzfiles.append(simpleaudio.WaveObject.from_wave_file(name))
    for fiel in waltzfiles:
        player = fiel.play()#playing the simpleaudio objects, fiel, from the list waltzfiles
        player.wait_done()
